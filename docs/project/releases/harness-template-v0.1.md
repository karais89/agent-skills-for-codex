...
- 판정일: 2026-04-26
- 배포 모델: `full`
- 태그명: `harness-template-v0.1`
- 태그 생성 여부: 생성
- 태그 대상: 이 릴리즈 문서가 포함된 커밋

현재 full 하네스 템플릿은 새 프로젝트나 하네스 문서 골격이 없는 기존 프로젝트에 적용 가능한 릴리즈로 판단한다. 이미 `README.md`, `AGENTS.md`, `ARCHITECTURE.md`, `docs/` 문서 체계를 가진 프로젝트에서는 충돌이 정상적인 안전 동작이며, 자동 병합 대상이 아니다.
...
- 태그가 가리키는 정확한 커밋은 `git rev-list -n 1 harness-template-v0.1`로 확인한다.
