...
외부 참고 저장소는 `reference-repos/` 아래에 클론하며, 이 디렉터리는 `.gitignore`에 포함되어 현재 프로젝트의 Git 이력에는 저장하지 않는다. 참고 저장소의 코드는 직접 복사하기보다 구조, 문서화 방식, 스킬 구성, 에이전트 지침을 파악하는 용도로 우선 사용한다.

## 저장소

| 용도 | 원격 저장소 | 로컬 경로 | 클론 기준 커밋 |
| --- | --- | --- | --- |
| Codex 참고 | https://github.com/openai/codex | `reference-repos/openai-codex` | `355c40a Support end_turn in response.completed (#19610)` |
| Karpathy 스킬 참고 | https://github.com/forrestchang/andrej-karpathy-skills | `reference-repos/andrej-karpathy-skills` | `2c60614 Sync Chinese README with English version (add Cursor section) (#95)` |
...
- 참고 저장소를 갱신할 때는 각 저장소 내부에서 `git pull` 또는 재클론을 수행하고, 필요한 경우 이 문서의 기준 커밋을 함께 갱신한다.
