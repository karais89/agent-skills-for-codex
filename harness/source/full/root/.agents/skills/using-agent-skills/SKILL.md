...
description: Routes work to the right local agent skill. Use at session start or whenever you need to decide which agent-skills workflow applies.
---
...
## Repository Harness Entrypoints

This repository may route short text aliases through `.codex/hooks/user_prompt_submit.py`. Treat those aliases as explicit requests to open and follow the matching wrapper skill:

| Alias | Wrapper skill | Primary artifact |
| --- | --- | --- |
| `spec`, `spec:` | `$harness-product-spec` | `docs/product-specs/<slug>.md` |
| `plan`, `plan:` | `$harness-exec-plan` | `docs/exec-plans/active/<slug>.md` |
| `build`, `build:` | `$harness-exec-build` | next unchecked item in an active ExecPlan |
| `test`, `test:` | `$agent-skills-test` | focused tests and verification |
| `review`, `review:` | `$agent-skills-review` | five-axis review |

Do not use the legacy root `SPEC.md`, `tasks/plan.md`, or `tasks/todo.md` workflow for `spec`, `plan`, or `build`.
