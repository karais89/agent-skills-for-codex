...
사용자가 대상 경로, 프로젝트 이름, 목적을 모두 먼저 채우게 하지 않는다. 에이전트는 현재 context로 기본값을 제안한 뒤, 새 프로젝트 생성 전에는 반드시 한 번 확인 질문을 한다.

기본값:

- 대상 경로: 현재 작업 디렉터리.
- 프로젝트 이름: 대상 디렉터리 이름을 사람이 읽기 좋은 형태로 변환한 값.
- 프로젝트 목적: 사용자가 말하지 않았으면 "아직 정하지 않음"으로 기록하고, README와 ARCHITECTURE에는 나중에 갱신할 수 있다고 명시한다.

새 프로젝트 생성 전 확인 질문:

```text
하네스를 새 프로젝트로 설정하기 전에 확인할게요.
- 대상 경로: 에이전트가 추론한 실제 경로
- 프로젝트 이름: 에이전트가 추론한 실제 이름
- 프로젝트 목적: 아직 정하지 않음 또는 사용자가 준 설명

이대로 진행할까요? 바꾸고 싶은 값이 있으면 경로, 이름, 목적만 알려주세요.
```

사용자가 동의하거나 변경값을 주면 진행한다. 사용자가 "목적은 나중에", "이름도 나중에"라고 답하면 임시값을 문서에 명시하고, 아래의 "프로젝트 프로필 갱신 절차"를 다음 단계로 안내한다.

추가 확인이 필요한 경우:

- 현재 작업 디렉터리가 하네스 소스 저장소 자체인 경우에는 현재 디렉터리에 적용하지 말고 대상 경로를 물어본다.
- 현재 작업 디렉터리가 비어 있지 않고 새 프로젝트 대상인지 불분명한 경우에는 기존 프로젝트 적용으로 전환할지 물어본다.
- 대상 경로에 기존 `README.md`, `AGENTS.md`, `ARCHITECTURE.md`, `docs/`가 있어 충돌이 예상되는 경우에는 dry-run 전이라도 먼저 알려준다.

질문은 한 번에 1-3개만 한다. 사용자가 "나중에", "기본값으로", "현재 폴더에"처럼 답하면 그대로 진행한다.

## 새 프로젝트 생성 절차

입력 처리:

- 대상 경로가 없으면 현재 작업 디렉터리를 사용한다.
- 프로젝트 이름이 없으면 대상 디렉터리 이름을 사용한다.
- 프로젝트 목적이 없으면 "아직 정하지 않음"으로 두고 문서에 후속 갱신 지점을 남긴다.
- 실제 파일 적용 전에 위 값을 사용자에게 한 번 확인한다.
...
2. 대상 경로를 확인한다.
   - 사용자가 경로를 주지 않았으면 현재 작업 디렉터리를 사용한다.
   - 존재하지 않으면 생성한다.
   - 존재하지만 비어 있지 않으면 새 프로젝트 생성으로 진행해도 되는지 확인한다. 기존 프로젝트로 보이면 기존 프로젝트 적용 절차로 전환할지 물어본다.
...
6. 대상 프로젝트 문서를 초기화한다.
   ...
   - 프로젝트 목적이 아직 정해지지 않았으면 목적을 꾸며 쓰지 말고 "아직 정하지 않음"으로 둔다.
...
새 프로젝트 생성 시 이름이나 목적을 나중에 정하기로 했다면, 사용자가 정한 시점에 이 절차로 프로젝트 프로필을 갱신한다.

사용자 요청 예:

```text
이 프로젝트의 이름을 Note Lab으로, 목적을 개인용 노트 작성과 완료 상태 관리를 실험하는 앱으로 설정해줘.
하네스 INSTALL.md의 프로젝트 프로필 갱신 절차를 따라 README.md, ARCHITECTURE.md, AGENTS.md, docs/validation.md를 갱신하고 검증해줘.
```

갱신 대상:

- `README.md`: 제목, 프로젝트 개요, 빠른 시작, 개발 명령, 프로젝트 구조 설명.
- `ARCHITECTURE.md`: 시스템 개요, 컨텍스트와 경계, 주요 구성요소, 데이터와 상태, 인터페이스, 알려진 제약.
- `AGENTS.md`: 프로젝트 맥락 섹션의 이름, 목적, 현재 검증 명령.
- `docs/validation.md`: 실제 검증 명령이 정해졌으면 반영하고, 아직 없으면 "아직 없음"을 유지한다.

규칙:

- 사용자가 제공한 이름과 목적을 우선한다.
- 근거 없이 앱 기능을 꾸며 쓰지 않는다.
- 아직 구현되지 않은 런타임, API, UI, 저장소는 "아직 없음" 또는 "아직 정하지 않음"으로 둔다.
- root `SPEC.md`, `tasks/plan.md`, `tasks/todo.md`, `docs/references`는 만들지 않는다.
- 갱신 후 config parsing, alias routing, forbidden path 확인을 실행한다.
- 변경 사항을 커밋할 수 있으면 `Update project profile` 메시지로 커밋한다.
...
- 대상 경로가 없으면 현재 작업 디렉터리를 사용한다.
- 프로젝트 이름이 없으면 기존 `package.json`, README 제목, Git repo 이름, 디렉터리 이름 순서로 추론한다.
- 프로젝트 목적이 없으면 기존 README나 코드 구조에서 추론하되, 근거가 부족하면 "확인 필요"로 기록한다.
