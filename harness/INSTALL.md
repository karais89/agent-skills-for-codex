...
사용자가 대상 경로, 프로젝트 이름, 목적을 모두 먼저 채우게 하지 않는다. 정보가 부족하면 에이전트가 현재 context로 합리적인 기본값을 정하고, 위험하거나 애매한 경우에만 짧게 질문한다.
...
- 현재 작업 디렉터리가 하네스 소스 저장소 자체인 경우.
- 현재 작업 디렉터리가 비어 있지 않고 새 프로젝트 대상인지 불분명한 경우.
- 대상 경로에 기존 `README.md`, `AGENTS.md`, `ARCHITECTURE.md`, `docs/`가 있어 충돌이 예상되는 경우.
- 사용자가 프로젝트 이름이나 목적을 나중에 정하겠다고 하지 않았고, 문서에 임시값을 남기는 것이 부적절해 보이는 경우.
